...
import (
	...
	"k8s.io/klog"
	"open-cluster-management.io/multicloud-integrations/maestroapplication"
...
func main() {
	...
	maestroSeverAddr, err := maestroapplication.GetServiceURL(mgr.GetClient(), "maestro", "maestro", "http")
	...
	maestroGRPCServerAddr, err := maestroapplication.GetServiceURL(mgr.GetClient(), "maestro-grpc", "maestro", "")
	...
	maestroWorkClient, err := maestroapplication.BuildWorkClient(ctx, maestroSeverAddr, maestroGRPCServerAddr)
	...
	if err = maestroapplication.SetupWithManager(mgr, options.SyncInterval, maestroWorkClient); err != nil {
