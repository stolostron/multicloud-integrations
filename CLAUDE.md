...
- `gitopsaddon/`: Addon agent
  ...
  - `monitoring-crds/`: Monitoring CRD stubs (`served: true`) for `servicemonitors`, `prometheuses`, and `prometheusrules`. Installed on non-OCP managed clusters to prevent operator crashes and ArgoCD CR `phase: Failed` from missing `monitoring.coreos.com/v1` APIs.
...
- **Monitoring CRD Stubs (non-OCP)**: The repo ships three `monitoring.coreos.com` CRD stubs in `gitopsaddon/monitoring-crds/` — `servicemonitors`, `prometheuses`, and `prometheusrules` — with `served: true` and `x-kubernetes-preserve-unknown-fields: true`. The gitopsaddon agent installs these on non-OCP managed clusters (Kind, EKS) via `applyCRDIfNotExists` during `installViaEmbeddedManifests`, using the same detect-before-apply pattern as the Route CRD stub. On OCP clusters (which already have the full Prometheus stack), the CRDs already exist so the addon skips installation. Without `servicemonitors` and `prometheuses`, the operator crashes (registers `controller-runtime` informer watches that fail with `"timed out waiting for cache to be synced"` → FATAL). Without `prometheusrules`, the operator pod stays running but the ArgoCD CR enters `phase: Failed` with `"no matches for kind PrometheusRule"` — the ArgoCD reconciler cannot complete, leaving the instance broken. **All three stubs MUST use `served: true`** (unlike the Route CRD stub which uses `served: false`). The Route CRD uses `served: false` because the operator has a dedicated discovery check (`IsRouteAPIAvailable`) that skips the watch entirely when the API is not served — no equivalent exists for monitoring types.
...
2. **Pod stability** — all Running pods in `openshift-gitops`, `openshift-gitops-operator`, and `open-cluster-management-agent-addon` have zero restarts (`verify_pods_stable`). This catches operator crash loops (e.g., missing monitoring CRD stubs on non-OCP), addon RBAC issues, and image pull failures that would otherwise go unnoticed.
3. ApplicationSet-driven app syncs to Healthy on at least one managed cluster
4. Clean deletion from hub triggers pre-delete hook
5. MCA is fully deleted (cleanup Job runs on each managed cluster)
6. All managed clusters remain accessible after cleanup
