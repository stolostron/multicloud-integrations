...
- **local-cluster guestbook known limitation**: The guestbook app deployment to `local-cluster` may not fully reconcile (ArgoCD agent app controller timing). `test-scenarios.sh` treats this as a warning, not a failure.
