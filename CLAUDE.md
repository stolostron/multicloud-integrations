...
- **Cluster Secret Resource Proxy URL**: In agent mode, the hub controller creates ArgoCD cluster secrets whose `server` field uses the **resource proxy** service URL format: `https://openshift-gitops-agent-principal-resource-proxy.<argocd-ns>.svc:9090?agentName=<cluster-name>`. The resource proxy is a sidecar in the principal pod that forwards API requests from the hub ArgoCD UI (live manifest, resource tree lookups) to the appropriate managed cluster agent. The `agentName` query parameter tells the proxy which cluster to target. Verify with: `kubectl get secret cluster-<cluster-name> -n openshift-gitops -o jsonpath='{.data.server}' | base64 -d`.
...
- **Destination-based mapping for ArgoCD agent Applications**: When using `argocd-agent` with `ARGOCD_PRINCIPAL_DESTINATION_BASED_MAPPING=true`, ApplicationSet templates must use `destination.name: '{{name}}'` (matching the agent name), NOT `destination.server: '{{server}}'`. Using `server` causes the principal to fail with `"Failed to get agent name for application"` because it routes by destination name.
