...
- **Addon RBAC**: The `gitops-addon` ServiceAccount uses a fine-grained `ClusterRole` named `gitops-addon` (not `cluster-admin`). Defined in `addonTemplates/addonTemplates.yaml` (static) and `addon_template_management.go` (dynamic, for agent mode). Includes `escalate`/`bind` on both `roles`/`rolebindings` AND `clusterroles`/`clusterrolebindings` (the addon deploys the ArgoCD operator's leader-election Role which grants `patch` on configmaps and `patch`/`delete` on leases — without `escalate` on namespaced roles, Kubernetes rejects the Role creation with "attempting to grant RBAC permissions not currently held"). Also includes `patch` on `configmaps` and `patch`/`delete` on `leases` directly, and `delete` on all resource types that the cleanup Job needs to remove. A separate `gitops-addon-cleanup` ClusterRole/ClusterRoleBinding is annotated with `addon-pre-delete` and included in the pre-delete ManifestWork, ensuring the cleanup Job retains RBAC even after the regular ManifestWork (which deploys the main `gitops-addon` ClusterRole) is deleted during MCA teardown.
- **ArgoCD Application Controller RBAC (Policy)**: On managed clusters, the ArgoCD application-controller needs cluster-wide permissions to create/manage application resources. The hub Policy must include a `ClusterRoleBinding` granting `cluster-admin` (or equivalent) to the ArgoCD application-controller ServiceAccount (e.g., `acm-openshift-gitops-application-controller` in `openshift-gitops`). Without this, Applications stay `OutOfSync`/`Missing` with `"services is forbidden"` / `"deployments.apps is forbidden"` errors. Users should patch the Policy to add this RBAC — see the "Agent Mode Application Deployment" section for examples.
...
- **Pod stability verification**: `verify_pods_stable` in `test-cycle-eks-ocp.sh` checks that ALL Running pods across `openshift-gitops`, `openshift-gitops-operator`, and `open-cluster-management-agent-addon` namespaces have zero restarts after a settle period. This catches operator crash loops (e.g., missing monitoring CRD stubs on non-OCP), addon RBAC issues, and image pull failures. Controlled by `POD_STABLE_SETTLE_SECS` (default: 30).
...
- `gitopsaddon/monitoring-crds/` — Monitoring CRD stubs (`servicemonitors`, `prometheuses`, `prometheusrules`) with `served: true`, installed by the gitopsaddon agent on non-OCP managed clusters. Without `servicemonitors`/`prometheuses`, the operator crashes (`controller-runtime` informer cache sync timeout → FATAL). Without `prometheusrules`, the operator pod stays running but the ArgoCD CR enters `phase: Failed` with `"no matches for kind PrometheusRule"` — the ArgoCD instance is broken. OCP clusters already have the full Prometheus stack so the addon skips installation.
