...
func uninstallGitopsAgentInternal(ctx context.Context, c client.Client, gitopsOperatorNS, gitopsNS string) error {
	klog.Info("Starting Gitops agent addon uninstall")
	...
	if err := deleteOperatorResourcesInternal(ctx, c, gitopsOperatorNS); err != nil {
...
func deleteOperatorResourcesInternal(ctx context.Context, c client.Client, gitopsOperatorNS string) error {
	...
	for _, rt := range resourceTypes {
		...
		err := c.List(ctx, list,
			client.InNamespace(gitopsOperatorNS),
			client.MatchingLabels{"apps.open-cluster-management.io/gitopsaddon": "true"})

		if err != nil {
			klog.Warningf("Failed to list %s in namespace %s: %v", rt.kind, gitopsOperatorNS, err)
			continue
		}

		for _, item := range list.Items {
			klog.V(1).Infof("Deleting %s/%s in namespace %s", rt.kind, item.GetName(), gitopsOperatorNS)
