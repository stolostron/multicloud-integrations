...
var AddGitOpsSyncRescToManagerFuncs []func(manager.Manager, int, string, int) error
...
func AddGitOpsSyncRescToManager(m manager.Manager, interval int, resourceDir string, searchBatchSize int) error {
	for _, f := range AddGitOpsSyncRescToManagerFuncs {
		if err := f(m, interval, resourceDir, searchBatchSize); err != nil {
			return err
		}
	}
