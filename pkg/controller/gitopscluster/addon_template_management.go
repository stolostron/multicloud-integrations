...
func (r *ReconcileGitOpsCluster) EnsureAddOnTemplate(gitOpsCluster *gitopsclusterV1beta1.GitOpsCluster) error {
	...
	// Get operator and agent images from GitOpsCluster spec with fallback to defaults
	operatorImage := ""
	...
	// Get agent image from spec, or use default
	agentImage := DefaultArgoCDAgentImage
	if gitOpsCluster.Spec.GitOpsAddon != nil && gitOpsCluster.Spec.GitOpsAddon.ArgoCDAgent != nil && gitOpsCluster.Spec.GitOpsAddon.ArgoCDAgent.Image != "" {
		agentImage = gitOpsCluster.Spec.GitOpsAddon.ArgoCDAgent.Image
	}
