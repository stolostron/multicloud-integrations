...
//
// local-cluster is skipped entirely: it's not agent-routed (see ensureLocalClusterSecret), so it
// never runs an argocd-agent that needs this CA.
//
// The hub has no direct API access to remote clusters, so it creates/updates ManifestWorks in the
// managed cluster namespaces; the work-agent applies them to the spoke. ManifestWorks are never
// deleted — they persist even if the CA secret is rotated.
